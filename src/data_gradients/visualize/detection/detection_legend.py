...
@dataclass
class LabelInfo:
    """Holds information about labels.
...
def add_to_row_or_create_new(rows: List[List[LabelInfo]], label: LabelInfo, image_width: int) -> List[List[LabelInfo]]:
    ...
    """
    if not rows or rows[-1][-1] + label.text_size[0] + 2 * MARGIN_SPACE > image_width:
        rows.append([label, label.text_size[0] + 2 * MARGIN_SPACE])
    else:
        rows[-1].append(label)
        rows[-1][-1] += label.text_size[0] + MARGIN_SPACE
    return rows
...
def get_label_rows(labels: List[LabelInfo], image_width: int) -> List[List[LabelInfo]]:
...
def draw_legend_on_canvas(image: np.ndarray, class_color_tuples: Iterable[Tuple[str, Tuple[int, int, int]]]) -> np.ndarray:
    ...
    :return: The canvas with the legend drawn on it.
    """
