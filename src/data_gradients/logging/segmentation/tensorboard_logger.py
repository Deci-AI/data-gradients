...
class SegmentationTensorBoardLogger(TensorBoardLogger):
    def _visualize(self, batch_data: SegBatchData):
        ...
        for i in range(num_samples_to_visualize):
            ...
            label = labels[i]
            if (
                label.shape[2:] != image.shape[2:]
            ):  # Just in case label is different from image
                resize = torchvision.transforms.Resize(label.shape[-2:])
                image = resize(image)

            idxs = (
                torch.arange(len(label)).view(len(label), 1, 1) + 1
            )  # +1 to avoid 'killing' channel zero
            label = idxs * label
            label = torch.sum(label, dim=0)
            label = torch.ceil(
                (label * 255) / torch.max(label) + 1
            )  # normalize such that max(label) goes to 255
